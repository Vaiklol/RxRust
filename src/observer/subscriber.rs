...
impl<T, P> Subscriber<T, P> {
    pub fn new(next: T, error: P) -> Subscriber<T, P> {
        Subscriber {
            next,
            error,
        }
...
impl<T, P, F, E> Observer<T, P, F, E> for Subscriber<F, E>
    where F: FnOnce(T),
          E: FnOnce(P) {
    fn next(self, next: T) {
        (self.next)(next)
...
impl<T, P, F, E> MutRefObserver<T, P, F, E> for Subscriber<F, E>
    where F: FnMut(T),
          E: FnMut(P) {
...
impl<T, P, F, E> RefObserver<T, P, F, E> for Subscriber<F, E>
    where F: Fn(T),
          E: Fn(P) {
